...
class OpinionMinerHMM:
    ...
    def _init(self):
        with open(self._tag_num_filepath) as f:
            for line in f:
        ...
        with open(self._init_filepath) as f:
            for line in f:
                splits = line.strip().decode('utf-8').split("\t")
                tag = splits[0]
                prob = float(splits[1])
        ...
        with open(self._emit_filepath) as f:
            for line in f:
                splits = line.strip().decode('utf-8').split("\t")
                tag = splits[0]
                prob = float(splits[1])
        ...
        with open(self._transition_filepath) as f:
            for line in f:
                splits = line.strip().decode('utf-8').split("\t")
                tag1 = splits[0]
                prob = float(splits[1])
    ...
    def train(self, corpus_filename=None):
        ...
        with open(self._hmm_train_corpus) as f:
            for line in f:
                if line.strip():
                    # split the line into the several splits
                    splits = common_lib.re_space_split.split(line.strip().decode('utf-8'))
        ...
        # write the tag num
        with open(self._tag_num_filepath, mode="w") as f:
            self._tags = tags_num
            for tag, num in sorted(tags_num.items()):
                f.write("%s\t%d\n".encode("utf-8") % (tag, num))

        # write the init probability
        with open(self._init_filepath, mode="w") as f:
            total = sum(init_num.values())
            for tag, num in sorted(init_num.items()):
                prob = float(num) / total
                self._init_prob[tag] = prob
                f.write("%s\t%.100f\t%d\n".encode("utf-8") % (tag, prob, num))

        # write the transition probability
        with open(self._transition_filepath, mode="w") as f:
            ...
            for tag1 in sorted(transition_num.keys()):
                ...
                for tag2 in sorted(tag_dict.keys()):
                    ...
                    prob = float(num) / total
                    self._transition_prob[tag1][tag2] = prob
                    f.write("%s\t%.100f\t%s\t%d\n".encode("utf-8") % (tag1, prob, tag2, num))

        # write the emit probability
        with open(self._emit_filepath, mode="w") as f:
            for tag in sorted(emit_num.keys()):
                ...
                for word in sorted(tag_dict.keys()):
                    ...
                    prob = float(num) / total
                    self._emit_prob[tag][word] = prob
                    f.write("%s\t%.100f\t%s\t%d\n".encode("utf-8") % (tag, prob, word, num))
